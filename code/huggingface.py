...
class HFLM(TemplateLM):
    ...
    def _model_generate(self, context, max_length, stop, **generation_kwargs):
        ...
        return self.model.generate(
            ...
            return_dict_in_generate=True,# Baixi added for KV
            use_cache=True,
            **generation_kwargs,
        )
    ...
    def generate_until(
        ...
        bx_count = 0 #Baixi
        os.environ["ITER_CT"] = str(bx_count) #Baixi
        os.environ['LENGTH'] = '0' #Baixi
        ...
        for chunk in chunks:
            ...
            os.environ['PREFILL'] = str(context_enc.shape[1]) #Baixi
            ...
            #Baixi try retrieve kv cache
            KV_cache = cont.past_key_values
            for i in range(len(KV_cache)):
                # print('layer {}, size {}'.format(i,KV_cache[i][0].size()))
                # print(KV_cache[i][0][0,0,0,:5])
                key_array = KV_cache[i][0].to(torch.float32).detach().cpu().numpy().astype(np.float32)
                value_array = KV_cache[i][1].to(torch.float32).detach().cpu().numpy().astype(np.float32)
                np.save('[Your Dir]/llama3_8b_gsm8k_batch1/iter_{}_layer{}_key.npy'.format(bx_count,i),key_array)
                np.save('[Your Dir]/llama3_8b_gsm8k_batch1/iter_{}_layer{}_value.npy'.format(bx_count,i),value_array)
            # print(KV_cache[0].shape)

            # cont_toks_list = cont.tolist()
            cont_toks_list = cont.sequences.tolist() #Baixi: need to change to this so it works
